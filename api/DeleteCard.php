...
if($username === false) {
	...
	if ($data->num_rows > 0) {
		...
		if($databaseConnection->query($query)) { // If query was successful
			...
			SendSingleError(HTTP_INTERNAL_ERROR, 'failed to delete card from database', ERRTXT_FAILED_QUERY);
		}
	}
	else{
		SendSingleError(HTTP_BAD_REQUEST, 'credit card with this number does not exist or it does not belong to this user', ERRTXT_FAILED_QUERY);
	}
}
